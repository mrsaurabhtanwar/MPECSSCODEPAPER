...
while penalty~\cite{HuRalph2004,Anitescu2005} and interior-point
methods~\cite{RaghunnathanBiegler2005,LeyfferLopezCalvaNocedal2006}
frequently encounter numerical instability near the feasible boundary. The recent MPECopt algorithm of Nurkanovi\'{c} and
Leyffer~\cite{NurkanovicLeyffer2025} is an open-source method with global convergence to B-stationary
points and strong performance on the MacMPEC (191 problems)
benchmark~\cite{Leyffer2000}; however, its implementation depends on
the commercial MATLAB environment and its strongest results rely on the
commercial solver Gurobi, and it has not yet been validated on the
MPECLib (92 problems)~\cite{MPECLib} or NOSBENCH (603 problems)~\cite{NurkanovicPozharskiyDiehl2024}
collections.

Significant gaps remain in the practical MPEC software landscape. Many
algorithms rely on commercial software (HSL, SNOPT, Gurobi), standard
Scholtes regularization lacks built-in mechanisms for diagnosing
stronger stationarity than C-stationarity, and reproducible
open-source benchmarking infrastructure is still comparatively scarce.
MPECSS is designed to close these implementation and reproducibility
gaps within a single open-source code base.

MPECSS uses the Scholtes framework while adding an implementation-aware
\emph{adaptive regularization update} driven by the homotopy
...
test is used as an S-stationarity diagnostic on the $G$- and
...
\emph{MPECSS algorithm:} A fully open-source, multi-phase
  ...
  diagnostic, multistart and restoration mechanisms, and a Phase~III
...
\emph{Multiplier sign test:} A fast algebraic procedure that checks the
  converted $G$- and $H$-multipliers on numerically biactive indices to
...
\emph{Open-source benchmark infrastructure:} A reproducible benchmark
  runner for MacMPEC, MPECLib, and NOSBENCH with 249-column CSV logging,
