...
regularization}~\cite{Scholtes2001}, which replaces the
...
often depend on commercial solvers like SNOPT~\cite{GillMurraySaunders2005SNOPT}. The
...
stronger stationarity than C-stationarity. MPECSS is designed to close these implementation and reproducibility gaps.

MPECSS uses the Scholtes framework while adding an implementation-aware
\emph{adaptive regularization update} driven by the
complementarity residual and an integrated \emph{multiplier sign test}~\cite{ScheelScholtes2000}. In the implementation, this sign
...
The main contributions of this paper are such that
\emph{MPECSS algorithm:} A fully open-source, multi-phase
  ...
  test, multistart mechanisms, and a Phase~III
...
\emph{Adaptive regularization control:} A monitoring-driven update of
  $t_k$ based on the complementarity residual, stable biactive-set
...
\emph{Open-source benchmark infrastructure:} A reproducible benchmark
  runner for MacMPEC, MPECLib, and NOSBENCH,
  wide-schema CSV logging (300+ columns),
  environment capture, process-level isolation for per-problem
  timeout enforcement, resume support (via \texttt{--resume}), graceful recovery from timeout, interrupt,
  and out-of-memory events. The solver auto-selects between
